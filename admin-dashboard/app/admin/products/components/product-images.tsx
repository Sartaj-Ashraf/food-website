...
export function ProductImages({ product, onUpdate }: ProductImagesProps) {
  ...
  const handleRemoveImage = async (imageUrl: string) => {
    ...
    try {
      const response = await customFetch.delete(`/products/${product._id}/images/${encodeURIComponent(imageUrl)}`)
  ...
  const handleDownloadImage = (imageUrl: string, index: number) => {
    ...
    link.download = `${product.title}-image-${index + 1}`
    link.target = "_blank"
